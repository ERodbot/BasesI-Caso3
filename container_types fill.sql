...
SELECT CEILING((RAND(CHECKSUM(NEWID())) * 4))*5, CONCAT('material', CEILING(RAND(CHECKSUM(NEWID())) * 1000)), CONCAT('brand', CEILING(RAND(CHECKSUM(NEWID())) * 1000)), CONCAT('model', CEILING(RAND(CHECKSUM(NEWID())) * 1000)), CEILING(RAND(CHECKSUM(NEWID()))*14)
FROM number_sequence
OPTION (MAXRECURSION 32767);
